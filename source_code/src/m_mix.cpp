...
void M_mix::generateCycle(int voice_index) {

	int l1, l3;
	  unsigned int l2;
	  float mixgain;
        int voice = voice_index;
	    for (l3 = 0; l3 < in_port_list.count(); l3++) inData [l3] = in_port_list.at(l3)->getinputdata(voice);

	    mixgain = gain * mixer_gain[0];
	    for (l1 = voice; l1 < voice + synthdata->blocksize; ++l1) {
	      for (l2 = 0; l2 < synthdata->cyclesize; l2++) {
	        data[0][l1][l2] = mixgain * inData[0][l1-voice][l2];
	      }
	    }
	    for (l3 = 1; l3 < in_port_list.count(); l3++) {
	      mixgain = gain * mixer_gain[l3];
	      for (l1 = voice_index; l1 < voice_index + synthdata->blocksize; ++l1) {
	        for (l2 = 0; l2 < synthdata->cyclesize; l2++) {
	          data[0][l1][l2] += mixgain * inData[l3][l1-voice][l2];
	        }
	      }
	    }
}
