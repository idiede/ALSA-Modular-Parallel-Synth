...
void M_ringmod::generateCycle(int voice_index) {

  int l1;
  unsigned int l2;
  int voice = voice_index;
    vcoData1 = port_M_vco1->getinputdata (voice);
    vcoData2 = port_M_vco2->getinputdata (voice);

    for (l1 = voice; l1 < voice + synthdata->blocksize; ++l1) {
      for (l2 = 0; l2 < synthdata->cyclesize; l2++) {
        data[0][l1][l2] = gain * vcoData1[l1-voice][l2] * vcoData2[l1-voice][l2];
      }
    }
}
