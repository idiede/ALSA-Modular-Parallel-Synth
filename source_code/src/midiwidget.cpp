...
QModelIndex MidiControllerModel::parent(const QModelIndex &child) const
{
    if (child.isValid() && child.internalPointer()) {
        ...
        __typeof__(rMidiControllers.constEnd()) c
...
void MidiWidget::addMidiController(MidiControllerKey mck)
{
    ...
    __typeof__(midiControllers.end()) c(midiControllers.end());
...
{
	__typeof__(midiControllers.constEnd()) c
...
{
	__typeof__(synthdata->activeMidiControllers) New =
    new __typeof__(*synthdata->activeMidiControllers);

  for (__typeof__(midiControllers.constBegin()) mc =
	 midiControllers.constBegin();
       mc != midiControllers.constEnd(); ++mc) {
    MidiControllerContext *amcc = NULL;

    for (__typeof__(mc->context->mcAbles.constBegin()) mca =
  ...
  __typeof__(synthdata->activeMidiControllers) old =
