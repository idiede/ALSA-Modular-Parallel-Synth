...
void M_sh::generateCycle(int voice_index) {
  ...
  int voice = voice_index;
    ...
    trigData = port_M_trig->getinputdata (voice);

    for (l1 = voice; l1 < voice + synthdata->blocksize; ++l1) {
      for (l2 = 0; l2 < synthdata->cyclesize; l2++) {
        if ( !gate && (trigData[l1-voice][l2] > triggerLevel)) {
          sample = inData[l1-voice][l2];
          gate = true;
        } else {
          gate = trigData[l1-voice][l2] > triggerLevel;
        }
        data[0][l1][l2] = sample;
