...
Module::Module(M_typeEnum M_type, int outPortCount, QWidget* parent,
  ...
  , sp_mutex()
  , cycles(synthdata->poly*2)
  , futures_vector()

{
  cv.reset();

  //cycles.reserve(synthdata->poly*2);//stop resize
...
void Module::portMemAlloc(int outports, bool poly)
{
    ...
    int voices = poly ? synthdata->poly : 1;//?? If this is always true then why???
    ...
    //reduce to only send back block size of requested index

    //reduce to only send back block size of requested index start without a block size each voice sent to a thread..
//    outdata = (float **)malloc(sizeof(float *));//3-D array
//    int outdata_size = synthdata->poly/synthdata->blocksize;
//    int out_size = outdata_size * synthdata->periodsize * sizeof(float);
...
//    memset(outdata[0], 0, out_size);
  ...
  //  int outdata_size = synthdata->poly/synthdata->blocksize;
  //  int out_size = outdata_size * synthdata->periodsize * sizeof(float);
  //  outdata[0] = (float **)malloc((outdata_size) * sizeof(float *));
  //  outdata[0][0] = (float *)malloc(out_size);
  //  memset(outdata[0][0], 0, out_size);

 // for (int l2 = 1; l2 < outdata_size; ++l2){ //what does this do??
 //        outdata[0][l2] = data[0][l2 - 1] +
 //        out_size ? synthdata->periodsize : 0);
 //   }
...
void Module::Delete()
{
  ...
  alive = false;
  for (__typeof__(midiControllables.begin()) mcAI = midiControllables.begin();
...
Module::~Module()
{
    ...
    //TODO add destructor for outdata..
...
float** Module::getData(int index, int v, Port *port)
{
    int voice = v;

 sp_mutex.lock();//shouldn't need this

	if(cycles.at(voice) == 0)
	{
        generateCycle(voice);
		cycles.at(voice) = 1;
		port->voiceArray[voice].outdata[0] = data[index][voice];//this can be over written by other threads
	}

 sp_mutex.unlock();

	 port->voiceArray[voice].outdata[0] = data[index][voice];
	//return data[index][voice];//port->voiceArray[voice].outdata[0];// = data[index][voice];//right I'm only returning the first block of the index.make

	return port->voiceArray[voice].outdata;
}
